...
export async function updateSession(request: NextRequest) {
  ...
  // Si no esta autenticado y NO esta en ruta de auth, redirigir a login
  if (!user && !isAuthRoute) {
    return NextResponse.redirect(new URL('/login', request.url))
  }

  // Si esta autenticado y esta en ruta de auth, redirigir a home
  if (user && isAuthRoute) {
...
- Rutas protegidas (configurable en proxy.ts)
- Action updateProfile() para editar perfil
- Post-login redirige a / (root, flexible para cualquier app)
